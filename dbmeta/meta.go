...
func LoadTableInfo(db *sql.DB, schemas *map[string]bool, dbTables []TableSchemaAndName, excludeDbTables []string, conf *Config) map[string]*ModelInfo {
	...
	for i, tableSchemaAndName := range dbTables {
		...
		if schemas != nil && !(*schemas)[tableSchemaAndName.TableSchema] {
			fmt.Printf("Skipping unmatched schema %v: %v\n", *schemas, tableSchemaAndName)
			continue
		}
